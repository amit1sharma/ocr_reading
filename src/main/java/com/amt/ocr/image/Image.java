...
public class Image {
    ...
    public Image(String docType, String imageAbsoluteSourcePath, String imageName) throws IOException {
        ...
        if(f.exists()) {
            f.deleteOnExit();
            Utils.deleteDirectoryStream(Paths.get(f.toURI()));
        }
        f.mkdirs();
    ...
    public float calculatePPI(){
        return Utils.calculateDPI(this._mat.height(), this._mat.width());
    }
...
//        save(_original_image, path);
        for(MatOfPoint m : contours){
            ...
            if(aspectRatio>=Utils.allowedAspectRatio(docType)  && rect.width>(_gray.width())/2){
...
//        Core.bitwise_not(source, source);
    ...
    public Image deskew_dotnet(){
        ...
        Double avgAngle = angle/lines.height();
        Utils.rotate(_mat, -avgAngle);
        return this;
    }

    public Image markAreaOfInterest(@NotNull List<MatOfPoint> contours) {
