	...
	public ExtractMRZResponse getTextFromImage(String cid, String docType, String binaryImageFP, String binaryImageBP) {
		...
		try{
			...
			if(isRequestValid && docType.equalsIgnoreCase(passportType) ){
				...
				if(!resultFP.equals(record.optional) || !record.validDocumentNumber || !record.validEidaNumber){
			...
			APPLOGGER.error("Please try a different image or pdf and verify the content of the request." , e);
			result = null;
		}
		if(null == result || "".equals(result.trim()) || (docType.equalsIgnoreCase(eidType) && (null == resultFP || "".equals(resultFP.trim())))){
